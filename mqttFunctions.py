	...
	if len(corners) > 0:
		for i in range(0, len(corners)):
			markers['markers'][i] = {'marker-id': int(ids[i][0]), 'camId': ac.camId,
											 'corners': {'1': {'x': float(corners[i][0][0][0][0]),
															   'y': float(corners[i][0][0][0][1])},
														 '2': {'x': float(corners[i][1][0][0][0]),
															   'y': float(corners[i][1][0][0][1])},
														 '3': {'x': float(corners[i][2][0][0][0]),
															   'y': float(corners[i][2][0][0][1])},
														 '4': {'x': float(corners[i][3][0][0][0]),
